  ...
  const amrex::Geometry& geom, const int max_coarsening_level)
{
  ...
  const amrex::Real small_buffer = 0.3 * dx;
  const amrex::Real large_buffer = 20.0;
  amrex::EB2::BoxIF box1(
    {AMREX_D_DECL(-large_buffer, 0.0 - large_buffer, -large_buffer)}, {AMREX_D_DECL(8.0 - cen_y, p2_y, large_buffer)}, true);
  const int cyl_axis = 2;
  amrex::EB2::CylinderIF cyl(
    cen_y - small_buffer, cyl_axis, {AMREX_D_DECL(8.0 - cen_y, cen_y, 0.0)}, true);
  auto channel = amrex::EB2::makeIntersection(cyl, box1);

  // The upper 'extension' of the channel
  // Straight part
  amrex::EB2::BoxIF box2(
    {AMREX_D_DECL(-large_buffer, p2_y, -large_buffer)}, {AMREX_D_DECL(3.8, 2.0 + large_buffer, large_buffer)}, true);
  auto channel2 = amrex::EB2::makeIntersection(channel, box2);
  // The tip.  Construct triangle from planes:
  amrex::EB2::PlaneIF plane1({AMREX_D_DECL(3.8, p2_y, 0.0)}, {AMREX_D_DECL( 0.0, -1.0,0.0)});
  amrex::EB2::PlaneIF plane2({AMREX_D_DECL(3.8, p2_y, 0.0)}, {AMREX_D_DECL( -1.0, 0.0, 0.0)});
  amrex::EB2::PlaneIF plane3({AMREX_D_DECL(4.2, p2_y, 0.0)}, {AMREX_D_DECL( 1.0,  2.0,0.0)});
  ...
  amrex::EB2::BoxIF box3({AMREX_D_DECL(-large_buffer, y_sep, -large_buffer)}, {AMREX_D_DECL(6.8, 1.0, large_buffer)}, false);
  auto channel4 = amrex::EB2::makeUnion(channel3, box3);
  // The tip. Construct triangle from planes:
  amrex::EB2::PlaneIF plane4({AMREX_D_DECL(6.8, y_sep, 0.0)}, {AMREX_D_DECL(0.0, -1.0, 0.0)});
  amrex::EB2::PlaneIF plane5({AMREX_D_DECL(6.8, y_sep, 0.0)}, {AMREX_D_DECL(-1.0, 0.0, 0.0)});
  amrex::EB2::PlaneIF plane6({AMREX_D_DECL(7.0, y_sep, 0.0)}, {AMREX_D_DECL(1.0, 2.0, 0.0)});
  amrex::EB2::PlaneIF plane7({AMREX_D_DECL(6.977, y_sep, 0.0)}, {AMREX_D_DECL(1.0, 0.0, 0.0)});
