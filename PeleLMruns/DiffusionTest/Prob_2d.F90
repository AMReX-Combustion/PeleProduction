...
!------------------------------------
  ...
  subroutine setupbc()bind(C, name="setupbc")
    ...
    use probdata_module, only : nzones, BL_AIR, BL_FUEL
    use probdata_module, only : T_in
...
! ::: -----------------------------------------------------------
  ...
  subroutine init_data(level,time,lo,hi,nscal, &
      ...
      use probdata_module, only : BL_FUEL, BL_AIR, Y_bc
      use probdata_module, only : T_in
...
!      write(6,*)" made it to initdata"
      ...
      do j = lo(2), hi(2)
        ...
        do i = lo(1), hi(1)
          ...
          do n=1,nspecies
             scal(i,j,FirstSpec+n-1) = Y_bc(n-1,BL_FUEL)*eta + (1.d0-eta)*Y_bc(n-1,BL_AIR)
          enddo
          scal(i,j,Temp) = T_in
