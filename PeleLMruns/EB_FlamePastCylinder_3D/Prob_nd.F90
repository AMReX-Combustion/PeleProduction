...
! ::: -----------------------------------------------------------
   ...
   subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(c)
      ...
      use probdata_module, only: T_mean, P_mean, xblob, yblob, radblob, MeanFlow, Ly, Lz
...
! ::: -----------------------------------------------------------
   ...
   subroutine init_data(level, time, lo, hi, nscal, &
      ...
      use mod_Fvar_def, only : Density, Temp, FirstSpec, RhoH, bathID, oxidID, fuelID
      use mod_Fvar_def, only : domnlo, domnhi
      use probdata_module, only: T_mean, P_mean, xblob, yblob, radblob, MeanFlow, Lz
...
! Local
      ...
      REAL_T  :: x, y, z, Yl(nspecies), Xl(nspecies), Patm, pmf_vals(nspecies+3)
      REAL_T  :: dist, tran_dist, tanh_dist
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               if ( x > 0.0015 ) then
                   dist = x + 0.01d0
                   tran_dist = dist * tan(12.0d0*3.1415d0/180.d0) &
                                * ( 1.0d0 + 0.0632 * sin(2*Pi*2*z/Lz) + 0.048 * sin(2*Pi*(z-.004234)/Lz) &
                                   + 0.034 * sin(2*Pi*3*(z-.00712435)/Lz) )            
                   tanh_dist = 0.5d0 * (1.0d0 - tanh((ABS(y)-tran_dist)/0.012d0)) + 2.5d0
               end if

               call pmf(tanh_dist,tanh_dist,pmf_vals,nPMF)
...
!------------------------------------
   ...
   subroutine setupbc()bind(C, name="setupbc")
      ...
      use mod_Fvar_def, only : bathID, oxidID, fuelID

      implicit none

      REAL_T :: Patm, Yt(nspecies)
      integer :: nPMF
      REAL_T  :: Yl(nspecies), Xl(nspecies), pmf_vals(nspecies+3)
      ...
      call pmf(2.5d0,2.5d0,pmf_vals,nPMF)
