...
! ::: ===========================================================
   ...
   subroutine xvel_fill (vel, v_lo, v_hi, &
      ...
      use probdata_module, only : turb_scale, splitx, xfrontw
...
! Local      
      ...
      if (do_flct.eq.1) then
         ...
         do k = v_lo(3), domlo(3)-1
            do j = v_lo(2),v_hi(2)
               do i = v_lo(1),v_hi(1)
                  eta1 = 0.5d0*(1.d0 - TANH(2.d0*(sqrt(xx(i)**2+yy(j)**2)-splitx)/xfrontw))
                  vel(i,j,k) = vel(i,j,k) + eta1*vfluc(i,j,1)*turb_scale
               enddo
            enddo
         enddo
      endif
      ...
      if (do_flct.eq.1) then
         ...
         do k = v_lo(3), domlo(3)-1
            do j = v_lo(2),v_hi(2)
               do i = v_lo(1),v_hi(1)
                  eta1 = 0.5d0*(1.d0 - TANH(2.d0*(sqrt(xx(i)**2+yy(j)**2)-splitx)/xfrontw))
                  vel(i,j,k) = vel(i,j,k) + eta1*vfluc(i,j,2)*turb_scale
               enddo
            enddo
         enddo
      endif
         ...
         if (do_flct.eq.1) then
            ...
            do k = v_lo(3), domlo(3)-1
               do j = v_lo(2),v_hi(2)
                  do i = v_lo(1),v_hi(1)
                     eta1 = 0.5d0*(1.d0 - TANH(2.d0*(sqrt(xx(i)**2+yy(j)**2)-splitx)/xfrontw))
                     vel(i,j,k) = vel(i,j,k) + eta1*vfluc(i,j,3)*turb_scale
                  enddo
               enddo
            enddo
         endif
...
!===========================================================
   ...
   subroutine vel_fill (vel, v_lo, v_hi, &
    ...
    integer i, j, k
    REAL_T  x(3), vel_bc(3)
    REAL_T  rho, Yl(0:NUM_SPECIES-1), T, h, eta1, eta_time

    integer lo(dim), hi(dim)

    write(*,*) "DOING TURB VELFILL ", turb_scale
    ...
    call filcc (v_lo,v_hi,domlo,domhi,delta,xlo,bc(1,1))
    call filcc (v_lo,v_hi,domlo,domhi,delta,xlo,bc(1,1))
    call filcc (v_lo,v_hi,domlo,domhi,delta,xlo,bc(1,1))
       ...
       do k = v_lo(3), domlo(3)-1
          ...
          do j = v_lo(2),v_hi(2)
             ...
             do i = v_lo(1), v_hi(1)
                ...
                call bcfunction(x,delta,3,1,time,.true.,vel_bc, rho, Yl, T, h)

                ! not sure if that's correct, how has bc changed?
                vel(i,j,k,1) = vel_bc(1)
                vel(i,j,k,2) = vel_bc(2) 
                vel(i,j,k,3) = vel_bc(3) 
       ...
       if (do_flct.eq.1) then
          ...
          do k = v_lo(3), domlo(3)-1
             do j = v_lo(2),v_hi(2)
                ...
                if ((bc(3,1)==EXT_DIR).and.(v_lo(3)<domlo(3))) vel(:,j,k,1) = vel(:,j,k,1) + vfluc(:,j,1)*turb_scale
                if ((bc(3,1)==EXT_DIR).and.(v_lo(3)<domlo(3))) vel(:,j,k,2) = vel(:,j,k,2) + vfluc(:,j,2)*turb_scale
                if ((bc(3,1)==EXT_DIR).and.(v_lo(3)<domlo(3))) vel(:,j,k,3) = vel(:,j,k,3) + vfluc(:,j,3)*turb_scale
             enddo
          enddo
       endif
    endif
