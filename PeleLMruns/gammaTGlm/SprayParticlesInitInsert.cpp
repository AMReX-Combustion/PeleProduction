...
{
  const int numGrids = pelelm->numGrids();
  Real part_rho = 1.;//1g/cm^3 = 1e3kg/m^3 //ProbParm::partRho;
  Real part_dia = 1e-2;//1e-3cm = 1e-5m  //ProbParm::partDia;
  Real T_ref = 300.0; //ProbParm::T0;
  const IntVect num_part = amrex::IntVect(AMREX_D_DECL(100, 100, 100)); //ProbParm::partNum;
...
#endif
    amrex::Print() << "PeleLM::pstateVel " << PeleLM::pstateVel << '\n';
	  ParticleType p;
	  p.id() = ParticleType::NextID();
	  p.cpu() = ParallelDescriptor::MyProc();
	  for (int dir = 0; dir != AMREX_SPACEDIM; ++dir) {
	    p.pos(dir) = part_loc[dir];
	    p.rdata(PeleLM::pstateVel+dir) = 0.;
	  }
	  p.rdata(PeleLM::pstateT) = T_ref; // temperature
	  p.rdata(PeleLM::pstateDia) = part_dia; // diameter
	  p.rdata(PeleLM::pstateRho) = part_rho; // liquid fuel density
	  for (int sp = 0; sp != SPRAY_FUEL_NUM; ++sp) {
	    p.rdata(PeleLM::pstateY + sp) = 0.;
	  }
	  p.rdata(PeleLM::pstateY) = 1.; // Only use the first fuel species
