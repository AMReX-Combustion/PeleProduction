...
#define EXTRAP_RHOH
#ifdef EXTRAP_RHOH
  int eComp = std::max((int)RhoH, std::max((int)Density, std::max((int)last_spec,(int)Temp) ) );
#else
  int eComp = std::max((int)Density, std::max((int)last_spec,(int)Temp) );
#endif
  int nComp = eComp - sComp + 1;
...
// Set flux, flux divergence, and face values for rho as sums of the corresponding RhoY quantities
      ...
      for (int d=0; d<BL_SPACEDIM; ++d)
     {
      ...
      FArrayBox ftmp(force.box(),1); ftmp.setVal(0);
...
#else
// Extrapolate RhoH
// Note that this requires that the nspecies component of force be the RhoH forcing

      state_bc = fetchBCArray(State_Type,bx,RhoH,1);      

      godunov->AdvectScalars(bx, dx, dt, 
                             D_DECL(  area[0][S_mfi],  area[1][S_mfi],  area[2][S_mfi]),
                             D_DECL( u_mac[0][S_mfi], u_mac[1][S_mfi], u_mac[2][S_mfi]), 0,
                             D_DECL(edgeflux[0],edgeflux[1],edgeflux[2]), nspecies+1,
                             D_DECL(edgestate[0],edgestate[1],edgestate[2]), nspecies+1,
                             Sfab, RHcomp, 1, ftmp, 0, divu, 0,
                             (*aofs)[S_mfi], RhoH, advectionType, state_bc, FPU, volume[S_mfi]);

// Compute Temp on faces, store in nspecies+2 component of edgestate[d]
      for (int d=0; d<BL_SPACEDIM; ++d)
      {
        const Box& ebox = amrex::surroundingNodes(bx,d);
        eR.resize(ebox,1);
        ...
        FArrayBox eH(ebox,1);
        eH.copy(edgestate[d],nspecies+1,0,1);
        eH.mult(eR,0,0,1);
        edgestate[d].setVal(400,ebox,nspecies+2,1);
        RhoH_to_Temp_DoIt(edgestate[d],eH,eY,ebox,0,0,nspecies+2,htt_hmixTYP);
      }
#endif
