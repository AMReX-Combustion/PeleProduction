...
! ::: -----------------------------------------------------------
   ...
   subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(c)
      ...
      use mod_Fvar_def, only : ac_hist_file, cfix, changemax_control, &
                               ...
                               pseudo_gravity
      use probdata_module, only : standoff, pertmag, rho_bc, Y_bc, FK_rad
...
! ::: -----------------------------------------------------------
   ...
   subroutine init_data(level, time, lo, hi, nscal, &
      ...
      use probdata_module, only : standoff, pertmag, FK_rad
...
! Local
      REAL_T  :: x, y, z, Yl(nspecies), Xl(nspecies), Patm, r, r0, theta
      REAL_T  :: pmf_vals(nspecies+3), y1, y2
      REAL_T  :: pert,Lx,Ly,Lz,Lr
      ...
      do k = lo(3), hi(3)
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
              ...
              r = sqrt( x*x + y*y )
              theta = atan(y/x)
              Lr =2*3.1415926*FK_rad
             ! pert = pertmag*(1.000 * sin(2*Pi*4*r/Lr) &
             !      + 1.023 * sin(2*Pi*2*(r-.004598)/Lr) &
             !      + 0.945 * sin(2*Pi*3*(r-.00712435)/Lr)  &
             !      + 1.017 * sin(2*Pi*5*(r-.0033)/Lr)  &
             !      + .982 * sin(2*Pi*5*(r-.014234)/Lr))

                pert = pertmag*(1.000 * sin(4*theta) &
              ...
              !  pert = pertmag*(1.000 * sin(2*Pi*4*x/Lx) &
               ...
               y1 = ( (FK_rad-r) - standoff - 0.5d0*delta(2) + pert)*100.d0
               y2 = ( (FK_rad-r) - standoff + 0.5d0*delta(2) + pert)*100.d0
